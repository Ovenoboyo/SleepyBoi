...
public class MapsActivity extends Activity implements OnMapReadyCallback, PermissionsListener{
    ...
    private static final String GOOGLE_PLACES_API_KEY = BuildConfig.ApiKey;
    private MapView mapView;
    public MapboxMap mapboxMap;
    private int count, clockMin, clockHour;
    public Double latitude, longitude, OriginLatitude, OriginLongitude;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Mapbox.getInstance(this, GOOGLE_PLACES_API_KEY);
        ...
        latitude = Double.longBitsToDouble(myIntent.getLongExtra("lat", 0));
        longitude = Double.longBitsToDouble(myIntent.getLongExtra("lng", 0));
        editMap = myIntent.getBooleanExtra("editMap", false);

        Button confirm = findViewById(R.id.confirm);

        if (!editMap) {
            confirm.setOnClickListener(view -> startService(BackgroundDistanceMatrixService.class));
        } else {
            confirm.setOnClickListener(view -> switchActivity(AddItemActivity.class));
        }
    }
    ...
    private void initSearchFab() {
        findViewById(R.id.fab_location_search).setOnClickListener(view -> {
            Intent intent = new PlaceAutocomplete.IntentBuilder()
                    .accessToken(Mapbox.getAccessToken() != null ? Mapbox.getAccessToken() : GOOGLE_PLACES_API_KEY)
    ...
    private PlaceInfoHolder setPlaceInfoHolder(LatLng latLng) {
        ...
        return new PlaceInfoHolder(addresses.get(0).getLatitude(), addresses.get(0).getLongitude(), addresses.get(0).getAddressLine(0));
    }
    ...
    private void switchActivity(Class activity) {
        ...
        intent.putExtra("lat", placeInfoHolder.getLatitude());
        intent.putExtra("lng",  placeInfoHolder.getLongitude());
    ...
    private void startService(Class Service) {
        ...
        intent.putExtra("lat", Double.doubleToRawLongBits(latitude));
        intent.putExtra("lng",  Double.doubleToRawLongBits(longitude));
        ...
        switch (requestCode) {
            ...
            case REQUEST_CODE_AUTOCOMPLETE:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    marker.setPosition(new LatLng(((Point) selectedCarmenFeature.geometry()).latitude(),
                            ((Point) selectedCarmenFeature.geometry()).longitude()));
                    marker.setTitle("" + ((Point) selectedCarmenFeature.geometry()).latitude() + ", " + ((Point) selectedCarmenFeature.geometry()).longitude());
                    break;
